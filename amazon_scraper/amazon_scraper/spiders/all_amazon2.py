...
class AmazonSpider(scrapy.Spider):
    ...
    def parse_product_page(self, response):
        ...
        if marca_cpu == ['INTEL']:
            if (match1_modelo := re.findall(r'(?i)i\d+[\s-]*\d{4,5}', titulo[0])) != []:
                cpu_modelo = [match1_modelo[0]]
            elif (match2_modelo := re.findall(r'(?i)(\d+th\s*gen).*?(i\d)', titulo[0])) != []:
                cpu_modelo = [match2_modelo[0][1] + ' ' + match2_modelo[0][0]]
            elif (match3_modelo := re.findall(r'(?i)celeron|xeon|pentium', titulo[0])) != []:
                cpu_modelo = match3_modelo
            else:
        ...
        disc_size = re.findall(r'(?i)(\d+\s*gb(?!.*ram)|\d+\s*tb(?!.*ram))(.*hdd|.*ssd|.*emmc)', titulo[0])
